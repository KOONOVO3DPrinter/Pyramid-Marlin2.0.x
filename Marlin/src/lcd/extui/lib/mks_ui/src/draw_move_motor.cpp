...
void lv_draw_move_motor(void)
{
  ...
  lv_refr_now(lv_refr_get_disp_refreshing());
	...
	lv_imgbtn_set_style(buttonXtype, LV_BTN_STATE_PR, &tft_style_lable_pre);
	lv_imgbtn_set_style(buttonXtype, LV_BTN_STATE_REL, &tft_style_lable_rel);
	...
	lv_obj_set_pos(buttonXtype,OTHER_BTN_XPIEL*3+INTERVAL_V*4,titleHeight+OTHER_BTN_YPIEL+INTERVAL_H);
